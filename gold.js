...
// @version      1000.MAX.1
// @description  XHR Prototype Hook, Bulletproof UI, Auto-Next, Native CUB Sync
// @match        *://*/*
...
(function() {
    ...
    // Динамический генератор ответов на основе реального дампа
    function getAuthPayload(url) {
        ...
        // Точная копия ответа для эндпоинта /users/get
        ...
        // Расширенный ответ для ядра и проверки статуса
        return {
        ...
        window.Lampa.Player.listener.follow('message', function(e) {
            if (e.type === 'ended') {
                setTimeout(function() {
                    ...
                    if (!p.playlist || !p.opened) return;
                    
                    var idx = p.playlist.indexOf(p.opened);
                    if (window.Lampa.Timeline && window.Lampa.Timeline.update) {
                        window.Lampa.Timeline.update(p.opened);
                    }
                    
                    if (idx !== -1 && idx < p.playlist.length - 1) {
                        p.play(p.playlist[idx + 1]);

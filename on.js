(function() {
  ...
  if (window.showy_pro_ru_runtime_loaded) return;
  window.showy_pro_ru_runtime_loaded = true;
        ...
        if (serial && episodes.length > items.length && !params.similars) {
          ...
          left.forEach(function(episode) {
            ...
            var html = Lampa.Template.get('lampac_prestige_full', {
              ...
              title: episode.name, quality: day > 0 ? txt : ''
            });
            var loader = html.find('.online-prestige__loader');
            var image = html.find('.online-prestige__img');
            ...
            if (episode.still_path) {
              ...
              loader.remove(); image.append('<div class="online-prestige__episode-number">' + formatEpisodeNumber(episode.episode_number) + '</div>');
            }
            html.on('hover:focus', function(e) {
              last = e.target; scroll.update($(e.target), true);
            });
            html.css('opacity', '0.5'); scroll.append(html);
          });
        }
            ...
            onSelect: function onSelect(a) {
              ...
              if (a.copylink) {
                if (extra.quality) {
                  ...
                  Lampa.Select.show({
                    title: Lampa.Lang.translate('settings_server_links'), items: qual,
                    onBack: function onBack() { Lampa.Controller.toggle(enabled); },
              ...
              if (a.subscribe) {
                Lampa.Account.subscribeToTranslation({
                  card: object.movie, season: params.element.season, episode: params.element.translate_episode_end, voice: params.element.translate_voice
  ...
  function startPlugin() {
    ...
    function registerShowyProManifest() {
      ...
      for (var i = 0; i < plugins.length; i++) {
        if (plugins[i] && plugins[i].component == manifst.component) {
          plugins[i].type = manifst.type; plugins[i].version = manifst.version; plugins[i].name = manifst.name;
          plugins[i].description = manifst.description; plugins[i].onContextMenu = manifst.onContextMenu; plugins[i].onContextLauch = manifst.onContextLauch;
          Lampa.Manifest.plugins = plugins; return;
        }
      }
      plugins.push(manifst); Lampa.Manifest.plugins = plugins;
    }

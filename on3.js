...
// @version      1000.8.0
// @description  CUB Sync, API Auto-Next, Tizen UI Fix, Button Hide, Filters
// @match        *://*/*
...
(function() { 
    ...
    // ============================================================
    // 2. ИНЖЕНЕРИЯ ИНТЕРФЕЙСА (CSS ДЛЯ TIZEN)
    // ============================================================
    ...
    function observeDOM() {
        ...
        var observer = new MutationObserver(function() {
            if (!document.getElementById('yt-premium-tizen-css')) injectTizenDesign();
        });
        observer.observe(document.documentElement, { childList: true, subtree: true });
    }

    // ============================================================
    // 3. БЕЗОПАСНЫЙ СЕТЕВОЙ ПЕРЕХВАТ
    // ============================================================
    ...
    if (window.fetch) {
        ...
        window.fetch = function(input, init) {
            try {
                ...
                if (url) {
                    if (checkMatch(url, CONFIG.AD_DOMAINS)) return Promise.resolve(new Response('{}', { status: 200 }));
                    if (checkMatch(url, CONFIG.PREMIUM_ENDPOINTS)) return Promise.resolve(new Response(JSON.stringify(getFakePremium()), { status: 200, headers: { 'Content-Type': 'application/json' } }));
                }
            } catch(e) {}
    ...
    // ============================================================
    // 5. ОПТИМИЗАТОР ДВИЖКА (НАСТРОЙКИ)
    // ============================================================
    var lastKeyTime = 0;
    window.addEventListener('keydown', function(e) {
        if (e.keyCode >= 37 && e.keyCode <= 40) {
            var now = new Date().getTime();
            if (now - lastKeyTime < 60) { e.stopImmediatePropagation(); e.preventDefault(); return false; }
            lastKeyTime = now;
        }
    }, true);
    ...
    // ============================================================
    // 6. ЧИСТЫЙ API-МОДУЛЬ АВТОПРОДОЛЖЕНИЯ (AUTO-NEXT PURE API)
    // ============================================================
    var _autoNextInited = false;
    function initAutoNext() {
        if (_autoNextInited || !window.Lampa || !window.Lampa.Player) return;
        
        window.Lampa.Player.listener.follow('message', function(e) {
            if (e.type === 'ended') {
                log('Видео завершено. Запуск API-логики автоперехода...');
                
                setTimeout(function() {
                    var player = window.Lampa.Player;
                    var pl = player.playlist;
                    var current = player.opened;
                    
                    if (pl && current) {
                        var currentIndex = pl.indexOf(current);
                        
                        // Сохраняем прогресс текущей серии через нативный API ядра
                        if (window.Lampa.Timeline && typeof window.Lampa.Timeline.update === 'function') {
                            window.Lampa.Timeline.update(current);
                        }
                        
                        // Если в массиве плейлиста есть следующий элемент
                        if (currentIndex !== -1 && currentIndex < pl.length - 1) {
                            log('Запуск следующей серии через Lampa.Player.play()');
                            var nextItem = pl[currentIndex + 1];
                            player.play(nextItem);
                            player.playlist = pl; // Реинициализация стейта
                        } 
                        // Если достигнут конец массива (Конец сезона)
                        else {
                            log('Конец плейлиста. Передача управления компоненту...');
                            player.close();
                            
                            // Обращаемся к API активного балансера (экрана), чтобы он подгрузил следующий сезон
                            var activity = window.Lampa.Activity.active();
                            if (activity && activity.component) {
                                if (typeof activity.component.nextSeason === 'function') {
                                    // Некоторые плагины (например, VMOD) имеют встроенную функцию следующего сезона
                                    activity.component.nextSeason();
                                } else {
                                    // Эмитируем нативный глобальный эвент для плагинов
                                    window.Lampa.Controller.trigger('next_season', current);
                                }
                            }
                        }
                    }
                }, 1000); // 1 сек задержка для корректного сохранения таймлайна в базу
            }
        });
        _autoNextInited = true;
    }
    ...
    log('✅ YouTube Premium Edition (Pure API v1000.8.0) загружен.');
